...
<Accordion title="Manual setup (without Mission Control)">
  If you prefer to configure the Gateway directly, create a `gateway.json` file:
  ...
  For secure token handling, use the `--secrets` flag:

  ```bash
  npm run gateway -- --config gateway.json --secrets secrets.json
  ```

  See [Channels](/channels#gateway) for the full configuration reference.
</Accordion>
...
The Gateway is a separate process that handles all messaging platform connections. It runs alongside your agent server and takes care of:
...
The Gateway is a client of the agent server — it connects to the Chat API (WebSocket, port 9101) just like any other client. Mission Control's built-in chat also connects through the Gateway using the same mechanism (WebSocket, port 9200).
...
<AccordionGroup>
  <Accordion title="Gateway configuration">
    ...
    Each channel specifies an adapter type and which agent to route to. The `agents` section defines WebSocket connection details for each agent server.
  </Accordion>
  <Accordion title="Running the Gateway manually">
    ...
    The secrets file is read once and deleted immediately. See [Channels](/channels#secrets) for the secrets file format.
  </Accordion>
  <Accordion title="Mission Control integration">
    ...
    3. Starts the Gateway process alongside the agent server
...
<CardGroup cols={2}>
  ...
  <Card title="Channels" icon="messages" href="/channels">
    Full reference for the Gateway config, adapters, and Chat API protocol.
  </Card>
  ...
  <Card title="Architecture" icon="sitemap" href="/architecture">
    How the Agent Server, Gateway, and Mission Control fit together.
  </Card>
</CardGroup>
