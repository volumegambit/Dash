...
Dash is made up of three pieces that work together to keep your agent team running.
...
Dash has three main components that can run on different machines:

- **Agent server** — The engine behind your agents. The agent server runs on your infrastructure and handles all AI interactions, tool execution, and conversation history. It reads your config, creates agents with their assigned models and tools, and starts two APIs: the Chat API (WebSocket, port 9101) for real-time interaction, and the Management API (HTTP, port 9100) for health checks, server info, and shutdown.
- **Gateway** — Connects your agents to the outside world. The gateway routes messages from platforms like Telegram to your agent team. It runs alongside the agent server, owns all channel connections, and routes messages to agents via the Chat API. Mission Control's chat panel also connects through the gateway over WebSocket. Every channel is handled the same way: one process, one config file, one message path to the agent.
- **Mission Control** — Your command center. Mission Control is a desktop app (and CLI) for deploying agents, monitoring their status, and chatting with them directly. Connects to the gateway for chat (WebSocket, port 9200) and to agent servers for monitoring (HTTP, port 9100). Includes a built-in TUI for terminal-based interaction.

### Deployment options

Dash is flexible about where each piece runs.

**Single machine** — run everything locally. Start the agent server and gateway, then use the TUI or Mission Control from `localhost`. Good for development and personal use.

**Multi-machine** — run the agent server and gateway on a VPS or private cloud, and use Mission Control from your laptop to manage and chat. The Chat API, Management API, and gateway are the only network-facing surfaces, each protected by its own auth token. You can manage multiple agent servers from a single Mission Control instance.
...
<AccordionGroup>
  ...
  <Accordion title="Agent server" icon="server">
    The main entry point. Reads config files and environment variables, creates agents with their assigned models and tools, and starts both the Chat API and Management API. Supports `--config` and `--secrets` CLI flags for production deployments. When deployed via Mission Control, secrets from the encrypted store are injected as a temporary `--secrets` file that is deleted after reading.
  </Accordion>
  <Accordion title="Gateway" icon="shuffle">
    A separate process that owns all channel connections. Reads a JSON config that maps channels to agents, then routes messages through the Chat API. The gateway is a client of the agent server — it connects to the Chat API like any other client. External chat platforms connect through platform-specific adapters: the Telegram adapter runs a bot that polls the Telegram Bot API for incoming messages, converts them into a common format, and forwards them to the agent. Mission Control's chat panel connects over WebSocket (port 9200) using the same adapter pattern. Adding a new platform means writing a new adapter — the routing and agent interaction stay the same.
  </Accordion>
  ...
  <Accordion title="Mission Control" icon="grid-2">
    Desktop app (Electron) and CLI for managing agent deployments across machines. Stores deployment records locally and secrets in an AES-256-GCM encrypted file (`~/.mission-control/secrets.enc`). Connects to remote agent servers via their management APIs. The CLI provides commands for deployment management (`deploy`, `status`, `stop`, `remove`, `logs`) and secret management (`secrets list`, `secrets set`, `secrets get`, `secrets delete`). The desktop app adds a visual interface with a secrets management page.
  </Accordion>
</AccordionGroup>
