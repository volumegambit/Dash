...
- **Gateway** — runs alongside the agent server. Owns all channel connections and routes messages to agents via the Chat API. External chat platforms like Telegram connect through the gateway — it runs a Telegram bot that polls for messages and forwards them to the agent. Mission Control's chat panel also connects through the gateway over WebSocket. Every channel is handled the same way: one process, one config file, one message path to the agent.
- **Mission Control** — runs on your local machine. A desktop app (Electron) or CLI for monitoring and managing remote agent servers. Connects to the gateway for chat (WebSocket, port 9200) and to agent servers for monitoring (HTTP, port 9100). Includes a built-in TUI for terminal-based interaction.
