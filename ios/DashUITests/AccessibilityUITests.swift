...
final class AccessibilityUITests: DashUITestCase {
  ...
  func testReduceMotionStreamingUsesStateNotAnimationTiming() {
    ...
    XCTAssertEqual(
      element("chat.subagent.ui-subagent", in: app).label, "Squad member researcher, Running")
  ...
  func testSemanticLabelsAndCompletedResponseExposure() {
    ...
    XCTAssertTrue(activity.label.contains("step"))
    scrollUntilHittable(activity, in: app).tap()
    XCTAssertEqual(
      element(
        "chat.message.assistant-ui-turn.activity.2.inspector.tool.ui-tool",
        in: app
      ).label,
      "Tool Search, Tool succeeded"
    )
