...
iPhone or iPad. The HQ remains authoritative: the app reads cached conversations offline,
reconciles when connectivity returns, and never runs an agent on the device.

## Supported scope

- Sign in with your Dash account and connect to any HQ enrolled through Desktop's
...
- Stream chat, resume interrupted turns, answer agent questions, cancel active turns, and attach
  supported images.
- Read and administer agents through the HQ-owned model catalog and configuration.
- Inspect connection health and remove one HQ's secrets, cache, and drafts from this device.

The app requires HQ capabilities `conversation-sync-v1` and `chat-resume-v1`, and treats
...
than failing the health decode, so a newer HQ never blocks an older install. It does not
provide provider credential UI, HQ shutdown controls, connector administration, or project
lifecycle controls.
...
- Node.js 22 when running the shared contract checks or integration harness with a real HQ.
...
Compile the integration suite for the HQ, then run deterministic UI coverage:
...
The runner requires the pinned iOS 18.4 iPhone, starts a fresh repository HQ harness for each
...
path end to end against a real (locally spun-up) control plane, relay, and HQ: a
...
scripted HQ, auth shim, TLS terminator) for the signer-device approval loop instead: this
...
round trip. Once signed in, `GatewayPickerView` lists every HQ enrolled in the account
(`ControlPlaneClient.listGateways`) by its relay subdomain. Tapping an HQ runs
`AccountConnectFeature.connect(to:)`: it mints a fresh pairing grant from the control plane for
that HQ, turns it into a relay `PairingPayload` (v2, port 443, both mobile tokens equal to
...
An HQ only appears in the picker once it has been enrolled from Desktop's
**Settings → Devices → Remote access**, which registers both the relay address and this app's
chat capability. An HQ enrolled before this app's account sign-in shipped has the relay
address but never registered the chat capability; tapping it fails instead of connecting. Opening
Desktop once on the machine that runs that HQ updates the HQ's app access
automatically — `healEnrolledGatewayChatToken` (`apps/mission-control/src/main/ipc.ts`) re-pushes
the chat token for any already-enrolled HQ whenever the local HQ launches, so no user action
beyond starting Desktop is needed.
...
| State | Exact copy |
| --- | --- |
| Control plane unreachable while loading HQs | Couldn't reach your Dash account service. Check your connection and try again. |
| Account has no enrolled HQs | No HQs linked to your account yet. Open Desktop → Settings → Devices → Remote access to enroll this machine. |
| HQ enrolled before this app's account sign-in shipped (`chatToken` absent) | This HQ needs to be re-enrolled from Desktop before app access works. |

`GatewayPickerView`'s toolbar has **Sign Out**, which tears down any active HQ connection,
best-effort revokes an abandoned mint, drops the cached account token, and returns to `SignInView`.

Before saving anything, the connect pipeline validates the payload, checks HQ health and
identity, and requires the mobile capabilities above. Connection secrets are then stored in
Keychain. SwiftData stores HQ-scoped profiles, conversations, messages, cursors, agents,
...
| State | Exact copy |
| --- | --- |
| Confirm sheet | Allow "<device>" to access <HQ>? |
...
| Signature/signer rejected (`403`) | Dash couldn't confirm this device is allowed to approve requests. Try again, or reconnect an HQ from Settings. |
...
| `Core/Persistence` | HQ-scoped SwiftData cache, replay cursors, drafts, insert-only pending sends, durable deletion revision floors, and external attachment data. |
| `Core/Sync` | Cache-first bootstrap, canonical reconciliation, tombstones, replay gaps, and reconnect backoff. |
| `Features/Account` | Account sign-in (Clerk-hosted browser flow), signer self-registration, and the HQ picker that lists enrolled HQs. |
...
| `Features/Settings` | Sanitized HQ status, reconnect, secure Disconnect & Forget, and approving other devices as a signer. |

The HQ is the source of truth. Cached rows remain readable during an outage, while canonical
...
A canonical deletion or not-found response raises a hidden HQ-scoped revision floor before
cached content is purged. Same-HQ reactivation preserves that floor, so stale or equal
...
saved record. An explicit retry reuses the original turn ID so HQ admission is idempotent while
...
Disconnect & Forget stops transports before deleting Keychain material, purging that HQ's
...
If the app reports **Update Dash**, the HQ advertises a newer incompatible mobile API or is
...
and tap the HQ again in the picker — the account connect path always mints a fresh grant, so
no QR code is involved.

### Rate limited (HTTP 429)

Dash shows the HQ-provided retry window and keeps cached content visible. Wait for the
countdown instead of repeatedly reconnecting or resending a mutation.

### Relay returns HTTP 502

A relay 502 normally means the relay cannot currently reach the HQ. Confirm the HQ is
online and registered with the relay, then use Reconnect. Only fall back to Disconnect & Forget
and reconnecting from the HQ picker if the relay credential or HQ identity changed.
...
exercise `Features/Pairing`'s verify + install machinery directly: confirm the iPhone and HQ
