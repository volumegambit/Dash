...
struct ChatConnectionTests {
  ...
  @Test("a parent-origin accepted frame registers a subscribed child turn")
  func parentAcceptedForSubscribedChildRegistersTurn() async throws {
    let childConversationID = "sub_01JYQ5X7N8ZK4TQ2M9V6C3B0AH"
    let serverTurnID = "018f0f4a-5c42-7a8b-9c01-a234567890ab"
    let task = FakeWebSocketTask()
    let connection = makeChatConnection(task: task)
    let sink = FrameSink()
    let collector = await collectFrames(from: connection, into: sink)
    try await connection.connect()
    try await connection.subscribe(agentID: "child-agent", conversationID: childConversationID)

    let accepted = try fixture("chat-accepted-subagent.json")
    let followUp = MobileWSServerFrame.event(
      id: serverTurnID,
      conversationId: childConversationID,
      seq: 2,
      event: .textDelta(text: "Parent guidance accepted.")
    )
    await task.enqueue(.string(serverJSON(accepted)))
    await task.enqueue(.string(serverJSON(followUp)))
    await waitForFrames(2, in: sink)
