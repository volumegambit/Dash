...
In Mission Control, open **Settings → Devices**. The **Pair Device** card can link the Dash iOS or
Android app over your local network, or through [Remote Access](/remote-access) after you enable it.
Scan the displayed code in the app, or use its paste/manual fallback.

The native iPhone and iPad client supports shared, resumable history and agent administration. See
[Dash for iPhone and iPad](/ios) for pairing, chat, agent-management, offline, and removal steps.
The Android app continues to provide its legacy private chat and monitoring experience until its
separate conversation-sync migration.
