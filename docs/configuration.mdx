...
Messages to a running child are capped separately, by `swarm.maxSteersPerWorker` (default `10`); the count spans a child's whole life, resumes included, for as long as the gateway keeps that child in memory. The count is not written to disk, so a child resumed after a gateway restart starts from zero again.

<Note>
  Mission Control has no Sub-agents card yet: `delegation`, `allowedTypes`, `allowedModels`, `maxDepth` and `modelAliases` are set through the [Management API](/api-reference) (`PUT /agents/:id`). Its **Configuration → Swarm** card edits the caps and the on/off switch — the switch reads and writes `subagents.enabled`, showing the same state the gateway acts on, so an agent that has neither block appears (correctly) as enabled.
</Note>
...
`openai/whisper-large-v3` for transcription, and OpenRouter's `openai/gpt-4o-mini-tts-2025-12-15`
with the `alloy` voice for speech.
...
<Note>
  A gateway upgrade that adds speech and an app upgrade that expects it need to be deployed
  together. An older gateway rejects a client's request to set dictation language to "Auto".
</Note>
