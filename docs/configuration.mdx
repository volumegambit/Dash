...
| `allowedModels` | *(unset)* | Models a sub-agent may be given. When unset, a child uses the parent's model and fallbacks. |
...
Messages to a running child are capped separately, by `swarm.maxSteersPerWorker` (default `10`); the count spans a child's whole life, resumes included.

<Note>
  Mission Control has no Sub-agents card yet. The `subagents` block is set through the [Management API](/api-reference) (`PUT /agents/:id`). Mission Control's **Configuration → Swarm** card still writes the older `swarm` block — and because it shows its checkbox unticked for any agent that has no `swarm` block, **saving that card turns sub-agents off** for the agent (`swarm.enabled: false`). Re-tick it, or set `subagents.enabled: true`, to turn them back on.
</Note>

A change to either block **takes effect on the agent's next message** — the gateway rebuilds the agent so the new limits, delegation mode, and enabled state apply to the next turn.

### Gateway defaults

The gateway enforces a **global** ceiling across every agent and supplies the per-agent defaults above. Each can be overridden with an environment variable set where the gateway runs:

| Setting | Default | Environment variable | Description |
|---------|---------|----------------------|-------------|
| `maxConcurrentWorkersGlobal` | `16` | `SUBAGENTS_MAX_CONCURRENT_GLOBAL` | Hard ceiling on sub-agents running **at once across all agents** on this gateway. A launch that would exceed it fails until others finish. |
...
`name` and `description` are required. `tools` is a comma-separated list of Dash tool names, MCP patterns (`mcp__<server>`, `mcp__<server>__<tool>`, `mcp__*`), or `agent(<type>, …)` to limit which types this agent may launch in turn; omit it to inherit everything the parent has. `disallowedTools` is subtracted first. `model` takes a `provider/model` id or `inherit`. `skills` names skills to preload into the prompt. `maxTurns` stops the agent after that many tool calls and reports what it has so far (it can be resumed). `background` makes this type detach by default. `isolation: worktree` gives the agent its own git checkout of the workspace, cut from the workspace's current commit. Claude Code keys Dash does not act on — `permissionMode`, `hooks`, `mcpServers`, `memory`, `effort`, `color`, `experimental` — are ignored with a warning in the log.
...
Plugin types are always namespaced, so they can never shadow anything; a workspace file named `Explore` does replace the built-in.
...
| `SUBAGENTS_*`, `SWARM_DEFAULT_*` | Per-agent sub-agent cap defaults — see [Gateway defaults](#gateway-defaults) for the full list. |
...
| `SubagentStart` | When a [sub-agent](/tools#sub-agents) starts (matched by its role) | Observe launches — log, notify, audit |
