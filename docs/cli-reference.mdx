...
Deployment abc123 created.
  Status: running
  Management API: http://localhost:9300
  Channel server: ws://localhost:9200/ws/chat
...
Sends a shutdown signal to the gateway. Does not remove the deployment record — use `mc status` to confirm it stopped, and `mc remove` to clean up.
...
Check the health of a Dash gateway.

```bash
mc health abc123                          # by deployment ID
mc health http://localhost:9300 -t $TOKEN # by URL
```
...
```bash
mc info abc123
mc info http://localhost:9300 -t $TOKEN
```
