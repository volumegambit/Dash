...
By default the Dash iOS or Android app connects to your gateway over your local Wi-Fi. The
...
Traffic is encrypted with TLS from your phone to the hosted relay and again from
...
and bodies while forwarding them. Pairing never gives a phone the gateway's
administrative token; it uses a phone-scoped capability and a revocable device credential.
</Note>
...
its own address, and every phone you pair carries its own revocable credential. The
gateway still enforces one phone-scoped capability for both the mobile API and chat;
the administrative bearer remains on the gateway machine.
...
<Steps>
  ...
  <Step title="Pair your phone by QR">
    Open **Settings → Devices**. With a gateway created, the QR code in the
    **Pair Device** section now carries your relay address and a per-device
    credential. Scan it with the Dash iOS or Android app — your phone or tablet connects
    through the relay from then on, wherever you are.
  </Step>
</Steps>
...
sync migration. Both apps use the same LAN/relay pairing security boundaries.
...
<Tip>
A revoked phone simply needs to be paired again to come back. Open **Settings →
Devices**, scan a fresh **Pair Device** QR code on that phone, and it gets a new
credential.
</Tip>
