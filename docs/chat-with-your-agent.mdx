...
The fastest way to talk to your agent. Open the **Chat** tab in Mission Control and start a new conversation.
...
When your agent uses a tool, it shows up as a card in the transcript. Cards look and behave the same way in Mission Control, the web client, and Dash for iOS.
...
The message box uses the same keys across Mission Control, the web client, and iOS, with one deliberate difference on iOS:

| Key | Mission Control & web | iOS |
|-----|--------------------------|-----|
...
Sessions are scoped by agent and conversation ID. A chat in Mission Control and a chat on Telegram are separate sessions, even if they're with the same agent.
