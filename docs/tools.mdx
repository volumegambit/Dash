...
<Tabs>
  ...
  <Tab title="send_message">
    ...
    Returns immediately. A **running** agent receives the message after its current step; a **finished** agent is resumed with its context intact and picks up from where it stopped. Either way, its next completion comes back as a notification.

    Refuses a one-shot type (`Explore`, `Plan`), an unknown name, and a message past the per-agent steer cap.
  </Tab>
  <Tab title="ask_orchestrator">
    ...
    The sub-agent shows as **waiting for input** until the parent replies with `send_message`, until you reply from the row's reply box, or until the request times out.
  </Tab>
</Tabs>
...
|------|---------------|-------|
| `general-purpose` | Researching complex questions, searching for code, multi-step tasks | Everything the parent has, except the skill-management, connector-management and Projects tools, which are never passed down |
...
`Explore` and `Plan` are **one-shot**: they are read-only and cannot be given a new task, so `send_message` refuses them. If one of them stops to ask a question, you can still answer it yourself from the reply box on its row.
...
- `send_to_worker` is `send_message`, which means it now **resumes a worker that has already finished** instead of refusing.
...
<Tabs>
  ...
  <Tab title="send_to_worker">
    ...
    Messages are capped per worker. A worker that has already finished is **resumed** by this call rather than refused; a one-shot type still refuses.
  </Tab>
