...
<Tabs>
  ...
  <Tab title="send_message">
    ...
    Refuses a one-shot type (`Explore`, `Plan`), an unknown name, and a message past the per-agent steer cap. The one-shot refusal is unconditional here: it applies even to a one-shot sub-agent that is paused on an `ask_orchestrator` question.
  </Tab>
  <Tab title="ask_orchestrator">
    ...
    Who can answer depends on the type. For `general-purpose` and for your own types, the parent answers with `send_message`. For `Explore` and `Plan` — the two **one-shot** built-ins — `send_message` refuses regardless of the question, so the answer has to come from the reply box on the sub-agent's row (or from the legacy [`send_to_worker`](#legacy-swarm-tools)).
  </Tab>
</Tabs>
...
`Explore` and `Plan` are **one-shot**: they are read-only and cannot be given a new task, so `send_message` refuses them — including when one of them is paused on a question. If one stops to ask, answer it yourself from the reply box on its row, or from the legacy `send_to_worker`, both of which do answer a paused one-shot. Your own types and plugin types are never one-shot.
...
- `send_to_worker` does what `send_message` does, which means it now **resumes a worker that has already finished** instead of refusing. The two differ in one place: `send_to_worker` also answers a **one-shot** worker that is paused on a question, where `send_message` refuses.
