...
The rest of this page is the reference behind each of those steps. Prefer files over clicks? You can also drop a plugin directory under `<dataDir>/plugins/` and edit `config.json` by hand — see [Configuration → Plugins](/configuration#plugins) and the [complete example](#a-complete-example-plugin) below.
