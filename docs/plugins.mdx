...
Here's the whole journey, from an empty list to a working plugin — all from **Mission Control → Plugins** (under **CONFIGURE** in the sidebar):
...
The plugin appears with a **status badge** — *Loaded*, *Disabled*, or *Error* — and a row of **contribution** tags showing what it adds (skills, commands, agents, mcp, hooks, providers, bin). It lands **enabled but untrusted**: its skills and commands work right away; its code components wait for you to trust it.
</Step>
...
The rest of this page is the reference behind each of those steps. Prefer files over clicks? You can also drop a plugin directory under your gateway's plugins folder (`~/.dash/gateway/plugins/`) and edit `config.json` by hand — see [Where plugins live](#where-plugins-live), [Configuration → Plugins](/configuration#plugins), and the [complete example](#a-complete-example-plugin) below.

## Where plugins live

Installed plugins live in your gateway's managed plugins folder:
...
This is the default. The gateway stores everything under `~/.dash` (overridable with `$DASH_HOME`), and its own data lives in `~/.dash/gateway` — so plugins land in `~/.dash/gateway/plugins/`. Running the gateway standalone with `--data-dir <path>` puts them in `<path>/plugins/` instead. See the [data directory layout](/configuration#data-directory) for the full picture.

You rarely need to touch this folder — Mission Control's **Plugins** screen and the management API manage it for you. But it's there if you want to drop in a plugin by hand or inspect `config.json` directly.
...
## Managing plugins in Mission Control

Mission Control's **Plugins** screen — under **CONFIGURE** in the sidebar — is where you install, inspect, enable, trust, and remove plugins without touching a config file. Changes take effect immediately; the gateway hot-reloads as you go.
...
| **Executables (`bin/`)** | Prepended to the gateway's `PATH`, so they resolve when a command, hook, or the agent's shell tools run them by name. | Yes |
...
A plugin's `commands/<name>.md` becomes a slash command **`/<plugin>:<command>`** — a plugin named `research` with `commands/triage.md` exposes `/research:triage`. A plugin's `agents/<name>.md` (Claude Code subagent format) becomes a loadable specialist invocable as **`/<plugin>:<agent>`**, or loaded by the agent through `load_skill`. Both are **markdown**, so they work on `enabled` alone — no trust needed. The plugin name namespaces them, so the slash form matches exactly with no ambiguity.

<Note>
**Subagents run in the main session**, not as isolated child agents. Their `tools` frontmatter is therefore advisory, not enforced — a loaded specialist can use the agent's full tool set. True isolated subagent spawning is a future enhancement.
</Note>
...
- **MCP servers** (`.mcp.json`) register with the gateway at startup. Their tools become part of the agent's toolset; registration is fail-isolated, so a server that won't start is logged and skipped without taking down the gateway.
...
- **Executables** (`bin/`) are prepended to the gateway's `PATH`, so a plugin's tools resolve ahead of system binaries when a command, hook, or the agent's shell tools invoke them by name.
...
In Mission Control, go to **Agents**, open the agent, and switch to its **Config** tab.
</Step>
...
Use **Add plugin…** to pick from the **loaded** plugins. Each one you add appears as a chip; remove a chip with its **×**. As soon as you select at least one, the agent is scoped to exactly those plugins. Clear all the chips to return to the **all-plugins** default.
</Step>
</Steps>

<Note>
Per-agent selection controls **visibility only — it does not trust a plugin**. Trust is gateway-wide: selecting a plugin for an agent never makes its code run if the plugin isn't trusted, and a trusted plugin stays trusted regardless of which agents are scoped to it. Only **loaded** plugins are offered in the picker — a disabled or errored plugin contributes nothing to route.
</Note>
...
**Install it.** From Mission Control's **Plugins** screen, put the folder's path in the **Source** field (for example `/Users/you/plugins/my-plugin`) and click **Install**. The plugin lands **enabled but untrusted** — the `summarize` skill and `/my-plugin:triage` command work right away. To turn on the provider and hook, click **Trust** and confirm. Then add a key for the provider on the [AI Providers](/ai-providers) screen, and its `myprov/my-fast` model becomes selectable for any agent.
