...
Here's the whole journey, from an empty list to a working plugin — all from **Mission Control → Settings → Plugins**:
...
Mission Control's **Settings → Plugins** screen is where you install, inspect, enable, trust, and remove plugins without touching a config file. Changes take effect immediately; the gateway hot-reloads as you go.
...
<Note>
**Plugin subagents run in the main session**, not as isolated child agents. Their `tools` frontmatter is therefore advisory, not enforced — a loaded specialist can use the agent's full tool set. For **isolated** parallel workers — each with its own context and a restricted tool subset — enable the agent's [Swarm](/configuration#swarm) feature instead.
</Note>
...
This is useful when an agent must only ever talk to a specific vendor — for example, keeping a customer-facing agent on Anthropic models only, regardless of which other providers are configured on the gateway.
