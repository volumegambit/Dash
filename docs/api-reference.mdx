...
`/mobile/v1`; native clients use the `/mobile/v1` paths.
...
| `POST` | `/speech/speech` | Synthesize speech, streamed as `audio/mpeg` |
...
streams MPEG audio. `text` is capped at 4,000 characters (`413 too_long`):
...
accept both `audio/mpeg` and `application/json` on this route.
