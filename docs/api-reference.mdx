...
Hands-free voice mode adds ten frames to the same `/ws/chat` socket used for resumable chat, gated
on the same `speech-v1` capability as the [Speech](#speech) routes. `id` is the client-generated
session id and is echoed on every `voice_*` frame in both directions.
...
| Type | Meaning |
|------|---------|
| `voice_state` | The session's phase: `listening`, `transcribing`, `thinking`, `speaking`, or `muted`. There is no `stopped` state here — see `voice_stopped` |
...
- There is no `voice_state stopped` frame; every session end is a `voice_stopped` frame instead.
