...
- **Gateway** — runs alongside the agent server. Owns all channel connections (Telegram, Mission Control chat) and routes messages to agents via the Chat API. One process, one config file for all channels. Mission Control's chat panel connects through the gateway — MC chat is just another channel.
...
<AccordionGroup>
  ...
  <Accordion title="Gateway" icon="shuffle">
    A separate process that owns all channel connections. Reads a JSON config that maps channels (Telegram, Mission Control chat) to agents, then routes messages through the Chat API. The gateway is a client of the agent server — it connects to the Chat API like any other client. Mission Control's chat panel connects to the gateway over WebSocket (port 9200).
  </Accordion>
