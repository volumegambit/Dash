...
|------|--------|-------------|
| `message` | `id`, `agentId`, `channelId`, `conversationId`, `text` | Send a chat message |
...
Conversations are stored as append-only JSONL files (one JSON object per line), under a directory keyed by agent and conversation:
