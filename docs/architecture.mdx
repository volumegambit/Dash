...
this release, so its private chat sessions are not part of that shared history.
...
**Turn-scoped, unless you ask for background.** A swarm run lives **within one chat turn**. The gateway's swarm coordinator keys each run by `agentId/conversationId`: only the live turn can spawn or finalize it, and when the turn ends the run is finalized and every still-running **foreground** worker is settled. A worker launched in the **background** is the exception: it keeps running after the turn ends, stays addressable by name, and reports back on its own. Each worker is bounded by its own wall clock (`maxRunSeconds`) whether or not its turn is still open, and cancelling a background worker (or deleting the conversation it belongs to) stops it.
...
**Supervision via the management API.** Mission Control's **swarm panel** reads live and recent runs over the [Management API](/api-reference) (`GET /agents/:id/swarm/runs` and `/runs/:runId`) and can cancel a worker or send it a message (`POST …/workers/:workerId/cancel` and `…/send`). A cancel or send against a worker that has already finished returns **409**, which the panel surfaces as a dismissable notice rather than an error. A "run" is the group of sub-agents one chat turn started, so runs are listed straight from the sub-agents' own conversations — they survive a gateway restart instead of disappearing with the process that created them.

**Sub-agents over HTTP.** The same three operations are available per conversation, on the loopback API and on `/mobile/v1`: `GET /conversations/:id/subagents` lists a conversation's sub-agents with their status and report, `POST /subagents/:id/stop` cancels one and everything it started, and `POST /subagents/:id/resume` sends it a message — resuming it if it has already finished. A sub-agent's transcript is a conversation of its own, so `GET /conversations/:id/messages` works on a sub-agent id too.

**After a restart.** A gateway that is killed mid-run marks every sub-agent that was running as **interrupted** on the next start, tells the parent agent about each one, and keeps the transcript. An interrupted sub-agent can be resumed — with `POST /subagents/:id/resume`, or by the parent agent itself — and picks up where it left off. Any leftover worktrees from sub-agents that were running are cleaned up at the same time, except ones holding uncommitted work, which are kept.
