...
<AccordionGroup>
  <Accordion title="A worker didn't spawn (cap reached)">
    ...
    **This is a cap doing its job.** The message tells you which one:
    ...
    Raise the relevant cap on the agent's **Swarm** card (or the gateway defaults) if the work genuinely needs more workers — see [Configuration → Swarm](/configuration#swarm). The change takes effect on the agent's next message.
  </Accordion>
