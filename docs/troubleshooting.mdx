...
<AccordionGroup>
  ...
  <Accordion title="Mobile request is unauthorized (401, 4001, or 4401)">
    ...
    **Fix:** Check each credential at its own boundary. `/mobile/v1` management requests use the
    management Bearer token; `/ws/chat` uses the separate chat token in `?token=`; a hosted relay
    additionally requires the paired device's relay credential. A relay credential does not
    replace either gateway token. Correct the mismatched token, or pair the phone again if its
    credential was revoked. Never paste tokens into logs or support messages.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="iPhone cannot reach a gateway on local Wi-Fi">
    **Symptom:** Pairing succeeds, but Dash for iOS cannot load the gateway while the phone and
    gateway computer are on the same Wi-Fi network.

    **Fix:** Open iOS **Settings → Privacy & Security → Local Network** and enable Dash. Return to
    Dash and retry. If Dash is already enabled, confirm both devices are on the same network and
    that the gateway is running. You can still use the paste/manual pairing fallback when camera
  ...
  <Accordion title="QR code still pairs over the local network">
    **Symptom:** The **Pair Device** section on **Settings → Devices** says it connects over your **local network** even though you want remote access.
  ...
  <Accordion title="Phone can no longer connect after a revoke">
    ...
    **Fix:** If that device was removed under **Settings → Devices → Paired devices** (by you or from another machine), its credential no longer works — this is expected. Pair it again: open **Settings → Devices**, scan a fresh QR code on the phone, and it gets a new credential.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="A worker didn't spawn (cap reached)">
    An agent with [Swarm](/configuration#swarm) enabled tried to spawn a worker and got an error instead. In chat this appears as a **red error tool block** on the `spawn_worker` call.

    **This is a cap doing its job.** The message tells you which one:

    - `too many workers running at once (max N) — wait for workers to finish` — the agent's **concurrent** limit. It should `wait_workers` before spawning more.
    ...
    Raise the relevant cap on the agent's **Swarm** card if the work genuinely needs more workers — see [Configuration → Swarm](/configuration#swarm). The change takes effect on the agent's next message. The gateway-wide ceiling across all agents (default `16`) is raised by setting the `SWARM_MAX_CONCURRENT_WORKERS_GLOBAL` environment variable where the gateway runs and restarting it — see [Configuration → Gateway swarm defaults](/configuration#gateway-swarm-defaults).
  </Accordion>

  <Accordion title="The swarm stopped partway (wall-clock limit)">
    Workers were cut off and the agent finalized the turn earlier than expected.

    **Fix:** A swarm turn has a wall-clock cap (`maxRunSeconds`, default **1800** = 30 minutes). When it's hit, the run closes, any in-flight workers are settled, and their cards terminalize. New spawns into a closed turn fail with `swarm turn is closed — cannot spawn`. If your workloads legitimately run longer, raise `maxRunSeconds` on the agent's **Swarm** card.
  </Accordion>

  <Accordion title="The gateway restarted mid-swarm-run">
    The gateway process died (crash, force-quit, machine reboot) while workers were still running.

    **Recovery is automatic.** On its next start the gateway detects the interrupted run and closes it out: every worker that never finished is marked **Cancelled** with the note `Gateway restarted while this worker was running.`, and the chat turn ends with the error `Gateway restarted while this swarm run was in progress — remaining workers were cancelled.` Once Mission Control reconnects, the conversation's worker cards stop spinning and show those terminal states, and the run appears as finished in the swarm supervision panel.

    Workers do not resume after a restart — re-send the prompt to run the swarm again.
  </Accordion>

  <Accordion title="Where are the worker transcripts?">
    Each worker runs its own session, stored **separately** from the agent's normal conversations so it doesn't clutter the conversation list:

    ```
    ~/.dash/gateway/sessions/<agent-name>/.swarm/<runId>/<workerId>/
    ```

    These transcripts are kept for inspection and are **never auto-deleted** — Dash does not garbage-collect them. If you want to reclaim space, remove old `.swarm/` run directories by hand:

    ```bash
    rm -rf ~/.dash/gateway/sessions/<agent-name>/.swarm
    ```
  </Accordion>

  <Accordion title="No swarm progress on Telegram / WhatsApp">
    You chat with a swarm-enabled agent over a messaging app and see only the final answer — no per-worker updates.

    **This is by design.** Live worker cards, the pinned strip, and the supervision panel are **Mission Control** features. Messaging channels deliver only the agent's **final response** (the answer it synthesizes from its workers), not the intermediate swarm events. To watch and steer a run in real time, chat with the agent in Mission Control.
  </Accordion>
</AccordionGroup>
