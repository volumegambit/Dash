...
<AccordionGroup>
  ...
  <Accordion title='alias "opus" is not configured (subagents.modelAliases)'>
    **Note on the tool result:**
    ...
    The `agent` tool's `model` parameter accepts short alias names as well as full model ids, but **no aliases are configured in this release** — there is no `subagents.modelAliases` setting yet. Any alias resolves to nothing, the sub-agent runs on the parent's model, and the note above is appended to the tool result. Nothing failed.

    **Fix:** pass a full `provider/model` id (or `inherit`) instead. If the id isn't the parent's model or one of its fallbacks, add it to the agent's `subagents.allowedModels` first — see [Configuration → Sub-agents](/configuration#sub-agents).
  </Accordion>
  ...
  <Accordion title="My sub-agent's worktree wasn't removed">
    ...
    **This is by design when the worktree holds work.** Cleanup removes a worktree only when there is nothing to lose: uncommitted changes, untracked files, and commits that no branch or tag would keep alive all keep it. (Regenerable litter — `node_modules/`, `dist/`, `build/`, `.next/`, `coverage/`, `*.log`, `.DS_Store` — does not count.) The path is not handed back to you in the chat: to find it, either fill in the template above with the sub-agent's id, or look in the gateway log, where every kept worktree is recorded with a `keeping <path>` line naming the path and why it was kept.
  ...
  <Accordion title="Resume is refused or greyed out">
    A **Resume** — from a row, from the tasks list, or from the parent's own `send_message` — comes back with an error instead. There are three reasons:

    - `Agent "..." is a one-shot Explore agent and cannot be resumed.` — `Explore` and `Plan` are read-only, one-shot types. **Fix:** launch a new one with the follow-up task. (If the agent is still running and asking a question, you *can* answer it from the reply box on its row — that's an answer, not a resume. The parent's own `send_message` refuses even then; the reply box and the legacy `send_to_worker` are the two paths that get through.)
    ...
    - `Agent "..." cannot be resumed: its grant cannot be rebuilt.` — the parent's grant could not be re-established. Either **sub-agents have been turned off** on the parent agent, or the parent conversation (or one above it) was **deleted** — deleting a conversation stops everything under it from running again. **Fix:** re-enable sub-agents on the parent agent — see [Configuration → Per-agent settings](/configuration#per-agent-settings), and read the note there about Mission Control's **Swarm** card, which is the usual way an agent ends up with sub-agents switched off. A deleted parent cannot be brought back, so launch a new sub-agent instead.
