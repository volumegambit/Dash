...
<AccordionGroup>
  ...
  <Accordion title="Dash for iOS says a device's code has expired">
    **Symptom:** Scanning another device's approval code from **Settings → Approve a
    device** shows:
    ...
    **Fix:** Approval codes expire a few minutes after they're generated. Ask whoever
    is waiting — on the browser or other device — to reload the gateway list (or try
    signing in again) to get a fresh code, then scan the new one right away. A code
    that's still valid instead shows a confirm sheet reading
    `Allow "<device>" to access <gateway>?` — tap **Approve** or **Deny** there.
  </Accordion>
</AccordionGroup>
