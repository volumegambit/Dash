...
<AccordionGroup>
  ...
  <Accordion title="My sub-agent's worktree wasn't removed">
    ...
    **This is by design when the worktree holds work.** Cleanup removes a worktree only when there is nothing to lose: uncommitted changes, untracked files, and commits that no branch or tag would keep alive all keep it. (Regenerable litter — `node_modules/`, `dist/`, `build/`, `.next/`, `coverage/`, `*.log`, `.DS_Store` — does not count.) The path is reported back with the sub-agent's result so you can find it.
  ...
  <Accordion title="Resume is refused or greyed out">
    ...
    - `Agent "..." is a one-shot Explore agent and cannot be resumed.` — `Explore` and `Plan` are read-only, one-shot types. **Fix:** launch a new one with the follow-up task. (If the agent is still running and asking a question, you *can* answer it from the reply box on its row — that's an answer, not a resume.)
    - `steer cap reached for "..." (max 10 per agent).` — you've hit the per-child message cap, which counts across resumes. **Fix:** raise `swarm.maxSteersPerWorker` for the agent, or start a fresh sub-agent.
    - A refusal naming a tool or model the sub-agent can no longer have — a resume re-checks the child's tools, MCP servers, and model against what the parent holds **now**, so a child cannot outlive a narrowing of its parent. **Fix:** restore the parent's tools or model, or launch a new sub-agent under the current configuration.
