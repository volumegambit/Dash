...
description: "A complete overview of how Dash's components fit together — agents, Mission Control, messaging apps, and more."
---
...
Mission Control is your command center for the entire system. It's a desktop app that spawns a background gateway process and gives you a UI to deploy agents, chat with them, monitor activity, and connect messaging platforms. Mission Control is also available as a CLI for terminal users and automation.
...
When you deploy an agent through Mission Control, the gateway creates it in-process with its own workspace. You can start, stop, restart, and remove deployments at any time. Configuration changes — model, tools, system prompt — can be made on the fly.
...
API keys and sensitive credentials are encrypted at rest using AES-256-GCM. The encryption key is derived from a password you set and cached in your OS keychain. Gateway management tokens also live in the OS keychain, never in plaintext files. Agents run in sandboxed workspace directories, and your data never leaves your machine except to your chosen AI provider.

[Learn more about Secrets](/secrets)

## Chat

Talk to your agents through Mission Control's built-in tabbed chat, through connected messaging apps like Telegram, or programmatically via the WebSocket API. Messages stream in real time — you see the agent thinking, using tools, and building its response as it goes. Conversations persist as sessions, so agents remember previous context.
