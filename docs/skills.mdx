...
Agents also pick up skills on their own. After a turn where the agent did real work, Dash runs a short review that asks the agent's own model one question: did this session teach anything worth keeping? Anything it finds is written into a skill the agent owns, so the next conversation starts already knowing it.

You don't have to do anything to switch this on — it is on by default.

### What gets learned

A lesson is one short, self-contained instruction: a technique that worked, a correction you made, a dead end worth avoiding next time. Lessons are grouped into skills named for a **kind of task** rather than for one job, so a handful of useful skills build up instead of a long list of near-duplicates.
...
Learned skills sit in the same managed directory as any other agent skill, are labelled **Agent** in Mission Control, and can be read, edited, or deleted there like the rest. Beside each one is a `lessons.json` file holding the individual lessons.
...
To correct something immediately, open the skill in Mission Control and edit or delete the lesson. To stop Dash managing a learned skill altogether, delete its `lessons.json` — the skill stays as an ordinary one that learning no longer touches.
...
| `approval` | `false` | When on, nothing is saved until you approve it. |
...
### Approving lessons before they are saved

Turn on `approval` when you'd rather see every lesson first. Proposals are then held instead of saved:

```bash
# List what is waiting
GET    /agents/{id}/skills/pending

# Save one
POST   /agents/{id}/skills/pending/{pendingId}/approve

# Discard one
DELETE /agents/{id}/skills/pending/{pendingId}
```

A held proposal is applied against your skills as they are **at the moment you approve it**, not as they were when it was proposed — so approving something days later won't undo changes you made in between.
