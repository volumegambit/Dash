 ...
 *
 * RETURNS the set of server names ACTUALLY registered (the addServer calls that
 * succeeded). A name skipped because it collided with a pre-existing operator
 * server is NOT in the returned set — so the caller's reload teardown removes
 * only servers the gateway owns and never an operator's identically-named server.
 */
 ...
 * ACTUALLY registered last time (the previous call's return value) — captured by
 * the caller BEFORE it swaps its live wiring reference, so we remove exactly what
 * we added (the new wiring may add, drop, or rename servers). Crucially, a
 * plugin-declared name that collided with an operator's server at registration is
 * NOT in this set, so we never tear down an operator-owned server.
 *
 ...
 *
 * RETURNS the set of names actually registered this pass — the caller persists it
 * as the next reload's `oldServerNames`.
 */
export async function reconcilePluginMcpServers(
  ...
  return registerPluginMcpServers(mcpManager, newConfigs, logger);
}
