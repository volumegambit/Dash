...
/**
 * Forward only end-to-end phone headers. The relay credential is consumed at
 * the public edge and must never reach the local gateway. WebSocket handshake
 * headers are regenerated by `ws`; forwarding the phone's originals would
 * create conflicting keys, versions, extensions, or hop-by-hop state.
 */
function downstreamHeaders(
  headers: Record<string, string>,
  websocket = false,
): Record<string, string> {
  const connectionScoped = new Set<string>();
  for (const [name, value] of Object.entries(headers)) {
    if (name.toLowerCase() !== 'connection') continue;
    for (const token of value.split(',')) {
      const normalized = token.trim().toLowerCase();
      if (normalized) connectionScoped.add(normalized);
    }
  }
  ...
  for (const [name, value] of Object.entries(headers)) {
    ...
    if (
      normalized === 'host' ||
      normalized === RELAY_CREDENTIAL_HEADER ||
      HOP_BY_HOP_HEADERS.has(normalized) ||
      connectionScoped.has(normalized) ||
      (websocket && normalized.startsWith('sec-websocket-'))
    ) {
      continue;
    }
...
export function startRelayClient(opts: RelayClientOptions): RelayClient {
    ...
    const req = http.request(
      {
        ...
        path: frame.path,
        headers: { ...downstreamHeaders(frame.headers), host: `127.0.0.1:${port}` },
      },
      (res) => {
        const conn = streams.get(frame.streamId);
  ...
  const openLoopbackWs = (
    ...
    // Authorization is forwarded verbatim (including malformed values) so the
    // gateway's header-first policy cannot be bypassed by a valid query token.
    // Browser-era query auth remains in frame.path as the absent-header fallback.
    const local = new WebSocket(`ws://127.0.0.1:${port}${frame.path}`, {
      headers: downstreamHeaders(frame.headers, true),
    });
