...
describe('swarm management routes', () => {
  ...
  describe('GET /agents/:id/swarm/runs/:runId', () => {
    ...
    it('does not respond until turn cancellation disposal settles', async () => {
      ...
      const request = Promise.resolve(
        app.request(`/agents/${id}/conversations/conv/swarm/cancel`, {
          method: 'POST',
          headers: AUTH,
        }),
      ).then((response) => {
  ...
  describe('POST /agents/:id/swarm/runs/:runId/workers/:workerId/cancel', () => {
    ...
    it('does not respond until worker cancellation disposal settles', async () => {
      ...
      let settled = false;
      const request = Promise.resolve(
        app.request(`/agents/${id}/swarm/runs/run-1/workers/worker-1/cancel`, {
          method: 'POST',
          headers: AUTH,
        }),
      ).then((response) => {
      ...
      try {
        ...
        vi.mocked(agents.evict).mockImplementation(async () => {
          ...
          expect(
            ...
            payload: { type: 'error', code: 'gateway_offline', retryable: true },
          });
        });
