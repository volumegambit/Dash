...
**Precondition:** App running, gateway healthy, at least one agent created, and the **Show the companion** toggle enabled (Settings → General → Companion). The companion is a separate always-on-top desktop window (not part of the main MC window); the in-app component is a headless publisher that streams session statuses and the selected pet to that window over IPC. The widget renders a **selectable pixel-art pet** — a **cat** (static grid sprite) or a **red panda** (default; **frame-animated**, PixelLab-generated). It shows one **aggregate mood** for all sessions. The cat shows mood via its collar-tag color plus eye overlays; the red panda plays a **per-mood animation** (idle sway / running / sitting / jumping / bristling) and shows the same mood hue as a small **collar badge dot**. Mood priority (highest wins): **error** (red `#f87171`) > **needs** (amber `#f5c518`) > **working** (blue `#3da5d9`) > **done** (green `#34c759`) > **idle** (gray `#9aa0a6` — no sessions).

### 30.1 Widget appears and floats
1. Launch MC with the companion enabled.
2. **Verify:** A small pixel-art pet widget appears at the **bottom-right** of the screen (frameless, transparent background, not shown in the taskbar/dock switcher). With no sessions running, the pet is **idle** (gray collar dot; the red panda plays its slow idle animation).
...
The widget shows a **single aggregate mood** across all sessions, not one indicator per session. Drive sessions into each state and observe the pet. For the **cat**, mood shows as collar color + expression; for the **red panda**, mood shows as a distinct animation + collar badge dot color. Test the priority ordering too.
1. With **no sessions** running.
2. **Verify:** The pet is **idle** — gray collar (cat: asleep; red panda: slow idle animation).
3. Start a long-running task so a session is **working**.
4. **Verify:** The pet shows the **working** mood — **blue collar** (cat: pulsing; red panda: running animation).
5. Ask a question the agent surfaces so a session **needs you** (unanswered).
6. **Verify:** The pet shows the **needs** mood — **amber collar** (red panda: sitting, waiting on you). (Needs outranks working: with both a working and a needs session, the pet is amber.)
7. Let a session **finish** while you are away (unread done), with nothing working or needing attention.
8. **Verify:** The pet shows the **done** mood — **green collar** (red panda: celebratory jumping).
9. Force a session to **error**.
10. **Verify:** The pet shows the **error** mood — **red collar** (red panda: bristling/angry animation). (Error is highest priority: with an errored session present, the pet is red regardless of any working/needs/done sessions.)

### 30.10 Animation quality and reduced motion
1. With the **red panda** selected and any mood active, watch the widget for ~10 seconds.
...
6. **Verify:** The red panda holds a **static frame** (first frame of the mood) instead of animating; the collar badge dot still shows the mood color.
7. In Settings → General → Companion, **verify:** the red panda thumbnail in the PetPicker plays its idle animation (static under reduced motion).
