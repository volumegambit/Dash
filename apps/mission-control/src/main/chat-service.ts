...
export class ChatService {
  ...
  /**
   * Replace a new conversation's placeholder title (the truncated first
   * message, set by `ConversationStore.appendMessage`) with a short title
   * generated by the gateway from that message. Fire-and-forget: any
   * failure — no management connection, network error, provider error —
  ...
  private async generateTitle(
    ...
    try {
      const res = await fetch(
        `${gc.managementBaseUrl}/agents/${encodeURIComponent(agentId)}/conversation-title`,
        {
          method: 'POST',
          headers: {
      ...
      await this.store.rename(conversationId, title.trim());
      this.onConversationRenamed?.(conversationId, title.trim());
    } catch {
  ...
  async sendMessage(
    ...
    // Auto-title: appendMessage just replaced the 'New Conversation'
    // placeholder with a truncated copy of this first message; upgrade it in
    // the background to a short LLM-generated title.
    if (conversation.title === 'New Conversation') {
      void this.generateTitle(conversation.agentId, conversationId, text);
    }
