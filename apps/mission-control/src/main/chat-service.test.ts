...
describe('ChatService conversation v2', () => {
  ...
  it('releases a v2 subscription when its renderer reloads', async () => {
    await service.subscribeV2(ref, bootstrap.v2ThroughSeq, 11);

    service.releaseConversationWatches(11);

    expect(transport.closeConversation).toHaveBeenCalledExactlyOnceWith(ref.id);
  });

  it('keeps a v2 subscription until its final renderer holder leaves', async () => {
    await service.subscribeV2(ref, bootstrap.v2ThroughSeq, 11);
    await service.subscribeV2(ref, bootstrap.v2ThroughSeq, 22);

    service.releaseConversationWatches(11);
    expect(transport.closeConversation).not.toHaveBeenCalled();

    service.unsubscribeV2(ref, 99);
    expect(transport.closeConversation).not.toHaveBeenCalled();

    service.unsubscribeV2(ref, 22);
    expect(transport.closeConversation).toHaveBeenCalledExactlyOnceWith(ref.id);
  });

  it('closes every v2 subscription when the window goes away', async () => {
    const secondRef = { ...ref, id: '00000000-0000-4000-8000-000000000002' };
    await service.subscribeV2(ref, bootstrap.v2ThroughSeq, 11);
    await service.subscribeV2(secondRef, bootstrap.v2ThroughSeq, 22);

    service.releaseAllConversationWatches();

    expect(vi.mocked(transport.closeConversation).mock.calls).toEqual([[ref.id], [secondRef.id]]);
  });
