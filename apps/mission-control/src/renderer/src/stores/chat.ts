...
function conversationViewFromSource(
  ...
  return {
    ...
    kind: summary.kind,
    ...(summary.parentConversationId ? { parentConversationId: summary.parentConversationId } : {}),
    ...(summary.parentTurnId ? { parentTurnId: summary.parentTurnId } : {}),
    ...(summary.subagent ? { subagent: summary.subagent } : {}),
...
export const useChatStore = create<ChatState>((set, get) => {
  ...
  const purgeConversation = (
    ...
    followSelectionSwitch(before.selectedConversationRef);
  ...
  const childConversationOwner = (subagentId: string): string => `subagent-live:${subagentId}`;
  const childConversationRef = (subagentId: string): ConversationRef => ({
    id: subagentId,
    origin: 'gateway',
  });
  /**
  ...
  /**
   * Child cards still consume the established v1 transcript shape. A capable-v2
   * child owns its own conversation socket, but its run frames describe the
   * same accepted/event/terminal lifecycle, so project them into that shape as
   * well as the durable v2 projection. Optional origin/correlation metadata is
   * copied when a newer gateway supplies it.
   */
  const childFrameFromV2 = (frame: MobileV2WsServerFrame): MobileWsServerFrame | null => {
  ...
  const clearSubagents = (): void => {
    ...
    for (const subagentId of [...childSubscriptions.keys()]) {
      const subscription = childSubscriptions.get(subagentId);
      childSubscriptions.delete(subagentId);
      if (subscription?.protocol === 'v2') {
        void get().closeConversation(
          childConversationRef(subagentId),
          childConversationOwner(subagentId),
        );
      } else {
  ...
  return {
      ...
      if (afterLookup.mainChatSurfaceActive) {
        ...
        const closing =
          previousRef && conversationKey(previousRef) !== key
            ...
            : Promise.resolve();
        await Promise.all([opening, closing, get().refreshSubagents()]);
        return;
      }
    ...
    cancelMessage(ref) {
      ...
      if (!source || !conversation) return;
      if (!turnId) {
        // A missed terminal frame can leave the optimistic sending flag set
        // after an authoritative read has already cleared activeTurnId. There
        // is nothing left to cancel remotely, so repair the local wedge.
        set((state) => ({
          sending: { ...state.sending, [key]: false },
          streamingFrames: { ...state.streamingFrames, [key]: [] },
          localTurnIds: { ...state.localTurnIds, [key]: undefined },
    ...
    async applyV2Frame(frame) {
      ...
      const selected = current.selectedConversationRef;
      if (knownChildIds.has(frame.conversationId) && frame.conversationId !== selected?.id) {
        const childFrame = childFrameFromV2(frame);
        if (childFrame) applySubagentFrame(frame.conversationId, childFrame);
      }
    ...
    subscribeSubagent(subagentId) {
          ...
          if (held.protocol === 'v2') {
            void get()
              ...
              .then(() => {
                ...
                if (current?.protocol === 'v2') current.live = true;
              })
              .catch(() => undefined);
          } else {
            window.api.subagentRewatch(held.agentId, subagentId);
          }
        }
        return;
      }
      ...
      if (!agentId) return;
      const parentProtocol = ref ? get().protocolByConversation[conversationKey(ref)] : undefined;
      if (parentProtocol === 'v2') {
        childSubscriptions.set(subagentId, {
          agentId,
          holds: 1,
          live: false,
          protocol: 'v2',
        ...
        void get()
          .openConversation(childConversationRef(subagentId), childConversationOwner(subagentId))
          .then(() => {
            const current = childSubscriptions.get(subagentId);
            if (current?.protocol === 'v2') current.live = true;
          })
      ...
      childSubscriptions.set(subagentId, { agentId, holds: 1, live: true, protocol: 'v1' });
      knownChildIds.add(subagentId);
