...
describe('ProviderConnectModal', () => {
  ...
  it('renders the provider title from PROVIDER_CONFIG', () => {
    render(
      <ProviderConnectModal provider="anthropic" onClose={noop} onSaved={noop} />,
    );
  ...
  it('calls onClose when Cancel is clicked', async () => {
    ...
    const onClose = vi.fn();
    render(
      <ProviderConnectModal provider="openai" onClose={onClose} onSaved={noop} />,
    );
  ...
  it('shows error when key name contains invalid characters', async () => {
    ...
    expect(
      await screen.findByText(
        'Key name must contain only letters, numbers, and hyphens.',
      ),
    ).toBeInTheDocument();
