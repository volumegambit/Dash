 ...
 * tool-call card is pure duplication — the card dumps the whole memory body,
 * description, name and type back out under a "Save Memory" header that sits
 * directly above the chip saying the same thing.
 *
 ...
 *
 * The distinction matters: a `save_memory` that hits the 200-memory cap
 * returns a NON-error result whose text is prefixed `Error:` and which
 * carries NO `memory` details, so no chip fires. Keying suppression off
 * `isError` alone would wrongly hide that card and make the failure vanish;
 * keying off the details block keeps it. */
