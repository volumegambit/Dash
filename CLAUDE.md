...
```bash
npm run speech:e2e                                        # auto-picks a model from ~/.dash/gateway/agents.json
SPEECH_E2E_MODEL=openrouter/openai/gpt-5.5 npm run speech:e2e   # or pin a model
```

`scripts/speech-e2e/run.mjs` boots a **real gateway** under an isolated temp `DASH_HOME` (reusing `scripts/memory-e2e/harness.mjs`) and drives the speech routes AND the hands-free voice WebSocket protocol against the **real** OpenRouter provider: (1) `GET /speech/config` shows the `openrouter` provider `available: true` and the `realtime` slot unavailable with a reason; (2) `GET /speech/models?kind=…` returns at least one transcription and one speech model; (3) `POST /mobile/v1/speech/transcriptions` on `fixtures/hello.wav` ("Hello from Dash, what is two plus two?") transcribes and the text contains "dash", "two"/"2", and "plus"/"+"; (4) `POST /mobile/v1/speech/speech` on a short sentence returns `audio/mpeg` bytes over 1 KB; (5) the validation envelope — an empty synthesis body answers `400 validation_failed`; (6) a full hands-free voice turn over `/ws/chat`: registers an agent, creates a conversation, opens its own WebSocket (not the shared harness's `driveTurn`, which drops non-turn frames), sends `voice_start`, waits for `voice_state listening` (the gateway drops `voice_audio` sent before that), then streams 600ms of calibration silence, `fixtures/hello.wav`'s raw PCM (found by walking the WAV's chunk list — `afconvert` inserts a filler chunk, so the data does not start at byte 44), and 1.5s of trailing silence as ~100ms-paced `voice_audio` frames. It asserts, in order: a final `voice_transcript` naming a `turnId`, the hub's `accepted` for that `turnId` arriving AFTER the transcript, `voice_state` visiting `transcribing` → `thinking` → `speaking`, at least one `voice_speech` with non-empty `audio`/`text` (`format: 'mp3'`, the default `minimax/speech-2.8-turbo` TTS), `done`, `voice_state listening` again, and finally `voice_stop` → `voice_stopped { reason: 'client' }`. It acknowledges every `voice_speech` with `voice_played { seq }` (a real client sends one once its playback chain drains) and asserts the second `listening` arrives AFTER that ack and less than 3s later — without that timing check the step would pass either way, since the gateway's 8s drain-safety timer advances the session on its own.

Prereqs: **Node ≥ 22.12** and a **provider API key configured** in the gateway (`~/.dash/gateway`). It makes real (small, ~cents) LLM + STT + TTS calls, so like `plugins:e2e` it is **not** part of `npm test`/`preflight`/CI. Run it after changes to `packages/speech/*`, the gateway speech routes (`apps/gateway/src/speech-routes.ts`), or the voice WebSocket wiring in `apps/gateway/src/chat-ws.ts`. Step 6 needs a model that actually answers a spoken question; a model that stalls or refuses fails it within the script's 120s budget for that step, and that is a real result about the model, not a broken script. It does **not** cover the iOS client, echo cancellation, speaker barge-in, mute, the realtime provider slot (no provider currently offers it), or any other client UI.
